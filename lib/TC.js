...
(function ($) {
	...
	var loaded_urls = {}, interface_source_cache = {};
	...
	function render_interface(url, parent, parameters) {
		...
		if (!TC.initialized) {
			TC.initialize(function () {
				TC.render_interface(url, parent, parameters);
			});
			return;
		}
		...
		load_controller(url, function (interface_source) {
			var path, root, controller, Constructor, i, l;
			...
			// Get the constructor that's named the same as the controller, but with "Controller" added.
			Constructor = root[path[l] + "Controller"];
			// Create a new instance of the controller.
			controller = new Constructor(parameters);
